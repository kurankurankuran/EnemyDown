...
struct TAssimpVertex {
	...
	TAssimpVertex()
	{
		...
		vTangent = DirectX::XMFLOAT3(0, 0, 0);
	}
};
...
public:
	...
	//const aiNode* GetNodebyBonebyName(std::string BoneName) const;
	const aiNode* GetNodebyBonebyName(std::string BoneName);
...
protected:
	...
	// aiNodeからSceneAnimNodeの情報を引き出せる
...
public:
	...
	void Draw(ID3D11DeviceContext* pDC, DirectX::XMFLOAT4X4& mtxWorld, EByOpacity byOpacity = eNoAffect/*, CAssimpModel * pMpdel = nullptr*/);
	void Release();
	...
	void SetAssimp(CAssimpModel* pModel) { m_pModel = pModel; }
private:
...
public:
	...
	void Draw(ID3D11DeviceContext* pDC, DirectX::XMFLOAT4X4& mtxWorld, EByOpacity byOpacity = eNoAffect/*, CAssimpModel * pMpdel = nullptr*/);
	void DrawNode(ID3D11DeviceContext* pDC, aiNode* piNode, const aiMatrix4x4& piMatrix, EByOpacity byOpacity/*, CAssimpModel * pMpdel = nullptr*/);
	...
	XMFLOAT4X4 GetBoneGlobalMatrix(std::string NodeName);
	//void SetVertex(TAssimpVertex* pVertex);
	
	void CopyAssimp(CAssimpModel* pModel);
...
private:
	...
	//void SetVertex(aiNode* piNode, TAssimpVertex* pVertex, UINT& nVtxCnt);
	void SetAssimp(CAssimpModel* pModel);
	void SetAssimpMesh(CAssimpModel* pModel, aiNode* pNode);
	const aiScene* GetaiScene() { return m_pScene; }
	SceneAnimator* GetAnimator() { return m_pAnimator; };
